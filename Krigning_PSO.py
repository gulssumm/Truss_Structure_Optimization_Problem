...
def pso_surrogate_assisted_algorithm(max_fe_limit, pop_size, kriging_model, scalers,
                                    eval_every_iter_true=1,  # minimum true evals per iteration
                                    stagnation_threshold=5,  # iterations without true FE -> force eval
                                    max_iterations=10000):    # absolute safety cap
    """
    Surrogate-Assisted PSO with safeguards:
    - ensures FE count advances,
    - forces true evaluations if surrogate never suggests improvements,
    - has absolute max_iterations cap to avoid infinite loops.
    """
    w = 0.7
    c1 = 2.0
    c2 = 2.0
    ...
    # INITIAL TRUE EVALUATION for swarm
    for particle in swarm:
        if fe_count >= max_fe_limit: break
        particle_fitness = calculate_fitness(particle['position'])
        fe_count += 1
    ...
    while fe_count < max_fe_limit and iteration < max_iterations:
        iteration_true_evals = 0  # count how many true FE in this outer iteration
        # compute surrogate predictions for whole swarm (cheap)
        surrogate_preds = []
        for particle in swarm:
            pred = kriging_predict_value(kriging_model, particle['position'], scalers)
            surrogate_preds.append(pred)

        # update particles
        for idx, particle in enumerate(swarm):
            if fe_count >= max_fe_limit: break

            # velocity & position update (requires gbest_position non-empty)
            if gbest_position is None:
                # safety fallback
                gbest_position = swarm[0]['position'][:]
        ...
        # Guarantee a minimum number of true evaluations per iteration (eval_every_iter_true)
        # If iteration_true_evals < eval_every_iter_true, force true evals on top surrogate-ranked particles
        if fe_count < max_fe_limit and iteration_true_evals < eval_every_iter_true:
            # rank particles by their surrogate predictions (ascending)
            ranked_idx = np.argsort(surrogate_preds)
            to_eval = int(eval_every_iter_true - iteration_true_evals)
            for j in range(min(to_eval, len(ranked_idx))):
                pi = swarm[ranked_idx[j]]
                true_fitness = calculate_fitness(pi['position'])
                fe_count += 1
                iteration_true_evals += 1
                if true_fitness < pi['pbest_value']:
                    pi['pbest_value'] = true_fitness
                    pi['pbest_position'] = pi['position'][:]
                    if true_fitness < gbest_value:
                        gbest_value = true_fitness
                        gbest_position = pi['position'][:]

        # Stagnation breaker: if no true eval happened for several iterations, force one random evaluation
        if iteration_true_evals == 0:
            stagnation_counter += 1
        else:
            stagnation_counter = 0

        if stagnation_counter >= stagnation_threshold and fe_count < max_fe_limit:
            # force evaluate best surrogate candidate (or a random particle if none)
            ranked_idx = np.argsort(surrogate_preds)
            candidate = swarm[ranked_idx[0]] if len(ranked_idx) > 0 else random.choice(swarm)
            true_fitness = calculate_fitness(candidate['position'])
            fe_count += 1
            if true_fitness < candidate['pbest_value']:
                candidate['pbest_value'] = true_fitness
                candidate['pbest_position'] = candidate['position'][:]
                if true_fitness < gbest_value:
                    gbest_value = true_fitness
                    gbest_position = candidate['position'][:]
            stagnation_counter = 0  # reset

        iteration += 1

        # Progress printing (periodic)
        if iteration % 50 == 0 or fe_count >= max_fe_limit:
            elapsed = time.time() - start_time
            print(f"[SA-PSO] iter={iteration}, fe_count={fe_count}/{max_fe_limit}, gbest={gbest_value:.4f}, elapsed={elapsed:.1f}s")

    # safety message if we capped iterations
    if iteration >= max_iterations:
        print(f"[SA-PSO] Warning: reached max_iterations={max_iterations} before FE limit.")
...
if __name__ == "__main__":
    ...
    best_fitness_sapso_1000, fe_count_sapso_1000, iterations_sapso_1000 = pso_surrogate_assisted_algorithm(
        FE_LIMIT_1000, POP_SIZE, kriging_model, scalers,
        eval_every_iter_true=1, stagnation_threshold=5, max_iterations=2000)
    ...
    best_fitness_sapso_5000, fe_count_sapso_5000, iterations_sapso_5000 = pso_surrogate_assisted_algorithm(
        FE_LIMIT_5000, POP_SIZE, kriging_model, scalers,
        eval_every_iter_true=1, stagnation_threshold=5, max_iterations=10000)
